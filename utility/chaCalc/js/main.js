...
function init(lib) {
	...
	xhr.open('GET', `/utility/chaCalc/data/${lib.toLowerCase()}.json`);
	xhr.onload = () => {
		if (xhr.status != 200) {
		...
		const contentclass = document.getElementById("chaCalcContent").classList;
		contentclass.remove("placeholder")
		contentclass.remove("bg-dark")
	...
	const contentclass = document.getElementById("chaCalcContent").classList;
	contentclass.add("placeholder")
	contentclass.add("bg-dark")
...
function _click(e) {
	const selected = e.target,
		lib = selected.textContent;
	document.querySelectorAll("#navLib a.nav-link").forEach((elm) => {
		if (elm == selected) {
...
document.querySelectorAll("#navLib a.nav-link").forEach((e)=> {
	e.onclick = _click
})
